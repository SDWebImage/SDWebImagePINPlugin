...
#import "PINCache+SDAdditions.h"
#import "PINMemoryCache+SDAdditions.h"
#import "PINDiskCache+SDAdditions.h"

static NSData * SDPINPluginCacheDataWithImageData(UIImage *image, NSData *imageData) {
...
@implementation PINCache (SDAdditions)
...
#pragma clang diagnostic pop
    ...
    if (shouldQueryDiskSync) {
        ...
        [self.diskCache objectForKeyAsync:key completion:^(PINDiskCache * _Nonnull cache, NSString * _Nonnull key, id  _Nullable object) {
            // PINDiskCache's completion block is called in the global queue
            NSData *diskData = nil;
...
- (void)storeImageToDisk:(UIImage *)image imageData:(NSData *)imageData forKey:(NSString *)key completion:(SDWebImageNoParamsBlock)completionBlock {
    NSData *data = SDPINPluginCacheDataWithImageData(image, imageData);
    if (!data) {
...
#pragma clang diagnostic pop
    ...
    [self.diskCache setObjectAsync:data forKey:key completion:^(PINDiskCache * _Nonnull cache, NSString * _Nonnull key, id<NSCoding>  _Nullable object) {
        if (completionBlock) {
            dispatch_async(dispatch_get_main_queue(), ^{
                completionBlock();
            });
...
- (void)removeImageForKey:(NSString *)key cacheType:(SDImageCacheType)cacheType completion:(SDWebImageNoParamsBlock)completionBlock {
    switch (cacheType) {
        ...
        case SDImageCacheTypeMemory: {
            ...
            break;
        case SDImageCacheTypeDisk: {
            [self.diskCache removeObjectForKeyAsync:key completion:^(PINDiskCache * _Nonnull cache, NSString * _Nonnull key, id<NSCoding>  _Nullable object) {
                if (completionBlock) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        completionBlock();
                    });
...
- (void)containsImageForKey:(NSString *)key cacheType:(SDImageCacheType)cacheType completion:(SDImageCacheContainsCompletionBlock)completionBlock {
    switch (cacheType) {
        ...
        case SDImageCacheTypeMemory: {
            ...
            break;
        case SDImageCacheTypeDisk: {
            [self.diskCache containsObjectForKeyAsync:key completion:^(BOOL contains) {
                if (completionBlock) {
                    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)clearWithCacheType:(SDImageCacheType)cacheType completion:(SDWebImageNoParamsBlock)completionBlock {
    switch (cacheType) {
        ...
        case SDImageCacheTypeMemory: {
            ...
            break;
        case SDImageCacheTypeDisk: {
            [self.diskCache removeAllObjectsAsync:^(PINDiskCache * _Nonnull cache) {
                if (completionBlock) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        completionBlock();
                    });
